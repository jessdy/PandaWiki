...
import { ConsultAttachment } from '@/request/ShareConsult';
 ...
 *   - 图片：≤ 20MB / 张，≤ 10 张
 *   - 视频：≤ 200MB / 个，≤ 3 个
 * 与后端 validateAttachments 闸门一致；后端会再次校验数量。
 */
const LIMITS = {
...
const AttachmentUploader = ({
  ...
  const handlePick = async (
    ...
    for (const f of Array.from(files).slice(0, remaining)) {
      // 前端类型校验避免误传：图片必须 image/*、视频必须 video/*
    ...
    try {
      ...
      for (const f of toUpload) {
        const res = await postShareV1CommonFileUpload({
          file: f,
          captcha_token: '',
        });
        // httpClient 返回 unwrapped data.key（与现有上传调用一致）
      ...
      message.error(
        (e as { message?: string })?.message || '上传失败，请稍后重试',
      );
    } finally {
      setUploading(false);
    }
  ...
  const triggerImg = () => imgInputRef.current?.click();
  const triggerVid = () => vidInputRef.current?.click();
